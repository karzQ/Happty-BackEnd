...
exports.get_all_users = async (req, res) => {
    ...
    try {
        User.find({},{select:{password: 0}}, (err, users) => {
            if (err) {
                statusCode = 500;
            ...
            User.findOne({email}, (err, user) => {
                if (err) {
                    ...
                    if (decryptPassword(req.body.password, user.password)) {
                        jwt.sign({email: user.email,userId:user._id, role: "user"}, JWT_TOKEN, {expiresIn: "1 hour"}, (err, token) => {
                            if (err) {
                                console.log({err})
                                statusCode = 500;
                                throw "Server internal error";
                        ...
                        return res.status(400).json({message: "The couple Email/Password is not working"})
